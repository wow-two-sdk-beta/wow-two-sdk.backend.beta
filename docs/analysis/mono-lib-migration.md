...
- **Folders now mirror namespaces (PascalCase, nested).** After the collapse, an IDE re-synced namespaces to the lowercase-kebab folder names (`data/ef-core/` → `…Beta.data.ef_core`), breaking ~89 files. Fixed by canonicalizing namespaces back to PascalCase and renaming folders to match: `Data/EntityFrameworkCore/Audit/`, `Identity/OAuth/Apple/`, `Foundation/Time/`, `Testing/Containers/Postgres/` (folder = namespace). Compound EF-Core / OAuth / Mfa / Migrations leaves are nested dirs, not kebab. A later pass (2026-06) PascalCased the remaining kebab area folders too — `comms/email-mailkit/` → `Comms/EmailMailKit/`, `http/auth-oauth2-client-credentials/` → `Http/AuthOAuth2ClientCredentials/`, `jobs/hangfire-postgres/` → `Jobs/HangfirePostgres/`, `meta/` → `Meta/`. A follow-up pass (2026-06-14) then converted those flat re-cased leaves to **nested/dotted** (provider-under-concept, matching `Identity/OAuth/Google/`): `Comms/EmailMailKit/` → `Comms/Email/MailKit/`, `Http/AuthOAuth2ClientCredentials/` → `Http/Auth/OAuth2ClientCredentials/`, `Jobs/HangfirePostgres/` → `Jobs/Hangfire/Postgres/`, and the empty stub families likewise (`Comms/Push*`, `Comms/Sms*`, `Messaging/Cap*`, `Jobs/Hangfire{Redis,SqlServer}`). The one all-caps folder `AI/` became `Ai/` (acronyms are always PascalCase, never all-caps — incl. `OpenAI` → `OpenAi`). The stale legacy `.sln` was dropped (the `.slnx` is canonical; the dual file made no-arg `dotnet restore` ambiguous). So every `src/` folder now matches its namespace 1:1.
  - **Foundation**: grouped under `Foundation/` folder; namespaces are fully nested (`Foundation/Time/` → `…Beta.Foundation.Time`) — folder = namespace 1:1, no top-level flattening.
  ...
  - **Empty scaffold areas** are also PascalCase + nested (provider-under-concept) for a uniform convention — e.g. `Ai/Vector/Qdrant/`, `Comms/Sms/Twilio/`, `Messaging/Cap/Kafka/`. They carry no `.cs` / lead-doc yet, so the nesting is filesystem-only until they get files.
