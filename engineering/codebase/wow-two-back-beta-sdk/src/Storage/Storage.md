...
*Provider-neutral blob (file object) storage — one `IBlobStorage` surface over the local filesystem now, cloud backends (S3 / Azure Blob / GCS) later.*
...
Cloud adapters on the same `IBlobStorage` surface — `Storage.S3` (AWSSDK.S3) · `Storage.Azure` (Azure.Storage.Blobs) · `Storage.Gcs` · a `FluentStorage` multi-cloud adapter · presigned-URL helpers · `ImageSharp` processing (Media).
