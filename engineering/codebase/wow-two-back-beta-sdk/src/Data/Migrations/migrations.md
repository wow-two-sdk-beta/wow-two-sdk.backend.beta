...
| **Multi-provider** | Any EF provider (PG/SQL Server/SQLite/…) | PG · SQL Server · MySQL via `DbUpProviderFactory` (SQLite needs custom factory) | **Postgres-only today** (advisory lock + transactional DDL); SQLite dialect is a planned drop-in seam |
