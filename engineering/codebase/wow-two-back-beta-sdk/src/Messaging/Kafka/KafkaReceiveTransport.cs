    ...
    MessageSerializerRegistry? serializerRegistry = null) : IReceiveTransport, IAsyncDisposable
{
    ...
    private void ConsumeLoop(Func<ReceiveContext, CancellationToken, ValueTask> onMessage, CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            if (envelope is null)
            {
                ...
                _deadLetterProducer!.ProduceAsync(options.Value.DeadLetterTopic, KafkaDeadLetterMapper.Build(result.Message, "unparseable", null), cancellationToken)
    ...
    {
        // A record re-consumed because its offset was never stored arrives byte-identical, so it reads as a first
        // delivery — the count drives give-up and second-level retry, and process-local state died in the rebalance.
